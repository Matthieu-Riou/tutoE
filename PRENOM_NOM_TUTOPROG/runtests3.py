...
import sys, doctest, string, dis, io
from colors3 import colorString
...
def prompt_release_stdout(fakestdout):
    ...
    while True:
        action = input("Print failed tests? [F(irst)/a(ll)/o(ne by one)/n(one)/p(ass this function)] ")
        action = action.lower()
        if action.lower() == 'p':
            return True
            ...
            for s in fakestdout.getvalue().split("*" * 70)[1:-1]:
                print(s)
                print
                action = input("Next error? [Y(es)/n(o)] ")
                action = action.lower()
                if action == 'n':
                        break
            break
...
if __name__ == '__main__':
    ...
    fakestdout = io.StringIO() # Fake file object for Stdout interception
